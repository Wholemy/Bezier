/*
 ...
 Этот исходный код предоставлен вам в целях ознакомления, вы можете его использовать как вам угодно, без каких либо гарантий)
 Код не является законченным и не претендует на звание лучшего, код выполняет конкретные задачи понятными для автора способами)
 Разработчик и автор этого исходного кода Весьмой Шум (Wholemy Noise) сайт автора: http://wholemy.ru
 P.S.: Я не нашел другого более подходящего мне кода в сети, и по этой причине решил создать его)
 Это код родился в моем уме, когда я изучал, перерабатывал и пытался использовать JavaScript аналог,
 не буду показывать пальцем, так как этот код с тем который я пытался использовать никак не связан)
 Код был разработан с целью замены DirectX комбайна, который создает кучу точек на параллельных кривых,
 причина по которой DirectX не способен соединить кривые мне известна, но исправить это не в моих силах,
 в моих силах показать как это возможно сделать или избежать, именно с этой целью существует данный код)
 Ошибки в DirectX 2D явились первичным мотиватором для разработки и этот код получается надежнее)
